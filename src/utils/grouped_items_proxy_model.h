...
/**
    ...
    GroupedItemsProxyModel allows a grouped item presentation of a flat table data model. Items can
   be visually grouped by data in one or more columns of the original data. Any number of groupings
   can be nested. GroupedItemsProxyModel inherits from QIdentityProxyModel and proxies all data from
   the original model unmodified. Only the visual presentation is affected, for example like \e
   QSortFilterProxyModel.

    Groupings can be managed with addGroup(), insertGroup() and removeGroup(). Each function takes a
   column number as a parameter. The source data is scanned, and a new grouping row is created for
   each unique value found in the given column. The title (and icon, if any) of the grouping row is
   taken from the source data (\c Qt::DisplayRole and \c Qt::DecorationRole respectively). By
   default, items are grouped based on their \c Qt::EditRole but you can change this with
   setGroupMatchRole().

    When grouping is active, a new first column is (optionally) added which can be used to sort the
   groupings. By default the groups are shown in the order in which they were found in the source
   data. The extra column can be disabled with setGroupColumnVisible(). The title of the column can
   be set with setGroupHeaderTitle().

    This model can be cascased with other models. For example set this model as the source for a \e
   QSortFilterProxyModel to enable sorting the view (including the grouping items as mentioned
   above).

    This model can only handle <b>flat data models</b> as input (table/list). It will not include
   any children of the top-level items.

    When no groupings have been added, this acts as a transparent proxy (see \e
...
public:
    ...
    /*! \property groupMatchRole Which data role to use for matching group data. Default is \c
     * Qt::EditRole. \sa setGroupMatchRole() */
    int groupMatchRole() const {
        return m_groupMatchRole;
    }

    /*! \property groupColumnVisible Show the extra grouping column (e.g. to allow sorting on it).
     * Default is \c{true}. \sa setGroupColumnVisible() */
    int groupColumnVisible() const {
        return m_groupColumnVisible;
    }

    /*! \property groupColumnIsProxy Proxy data from source column to the extra grouping column (if
     * shown). This includes display data. Default is \c{false}. \sa setGroupColumnIsProxy() */
    int groupColumnIsProxy() const {
        return m_groupColumnIsProxy;
    }

    /*! \property groupColumnProxySrc Which column to use for proxy data. A value of \{-1} (default)
     * means to use the grouping column. \sa setGroupColumnProxySrc(), groupColumnIsProxy() */
    int groupColumnProxySrc() const {
        return m_groupColumnProxySrc;
    }

    /*! \property groupRowSelectable Allow selecting the grouping row. Default is \c{false} \sa
     * setGroupRowSelectable() */
    int groupRowSelectable() const {
        return m_groupRowSelectable;
    }

    /*! \property groupHeaderTitle Title of grouping column. Default is \c "Group". Set an empty
     * string to hide title. \sa setGroupHeaderTitle() */
    QString groupHeaderTitle() const {
        return m_root->data(Qt::DisplayRole).toString();
    }

public Q_SLOTS:
    /*! Add a new grouping based on \a column. \sa removeGroup() */
    void addGroup(int column) {
        addGroups(QVector<int>() << column);
    }
