...
std::unordered_set<ConnectionId> SimpleGraphModel::allConnectionIds(const NodeId nodeId) const {
    ...
    std::copy_if(
        ...
        [&nodeId](const ConnectionId& cid) { return cid.inNodeId == nodeId || cid.outNodeId == nodeId; }
    );
...
QVariant SimpleGraphModel::portData(NodeId nodeId, PortType portType, PortIndex portIndex, PortRole role) const {
    switch (role) {
        case PortRole::Data:
            return QVariant();
            break;
...
bool SimpleGraphModel::setPortData(
    NodeId nodeId, PortType portType, PortIndex portIndex, const QVariant& value, PortRole role
) {
    Q_UNUSED(nodeId);
    Q_UNUSED(portType);
    Q_UNUSED(portIndex);
...
QJsonObject SimpleGraphModel::saveNode(const NodeId nodeId) const {
    ...
    {
        const QPointF pos = nodeData(nodeId, NodeRole::Position).value<QPointF>();
